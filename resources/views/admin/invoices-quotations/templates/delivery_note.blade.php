...
// Base64 encoded Raines logo (white version for header)
@endphp
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Delivery Note - {{ $document->document_number }}</title>
    <style>
        /* =========================================================
    ...
    <div class="container">
        ...
        <div class="info-section">
            <div class="info-column">
                ...
                @if($document->customer_email)<div class="info-line">{{ $document->customer_email }}</div>@endif
                @if($document->customer_phone)<div class="info-line">{{ $document->customer_phone }}</div>@endif
                @if($document->customer_address)<div class="info-line">{{ $document->customer_address }}</div>@endif
            </div>
            <div class="info-column" style="text-align: right;">
                <div class="section-title">QUOTATION DETAILS</div>
                <div class="info-line"><span class="label">Issue Date:</span> {{ $document->issue_date->format('M d, Y') }}</div>
                @if($document->valid_until)
                <div class="info-line"><span class="label">Valid Until:</span> <strong style="color: #e70810;">{{ $document->valid_until->format('M d, Y') }}</strong></div>
                @endif
                <div class="info-line"><span class="label">Currency:</span> {{ $document->currency_code }}</div>
            </div>
        </div>
        ...
        <div class="totals-section">
            ...
            <div class="totals-row">
                <span>Discount @if($document->discount_type == 'percentage')({{ number_format($document->discount_value, 2) }}%)@endif:</span>
        ...
        <p class="validity-notice">
            <strong style="color: red; padding-left: 12px;">Important:</strong> This quotation is valid until <strong style="color: #e70810;">{{ $document->valid_until->format('F d, Y') }}</strong>.
        ...
        <div class="footer">
            <p>Thank you for considering Raines for your needs!</p>
            <p>Raines Technologies (PTY) LTD | <span class="footer-accent">www.raines.africa</span></p>
            <p style="margin-top: 10px;">This quotation is computer-generated and valid for the period specified above.</p>
        </div>
    </div>
</body>
</html>
