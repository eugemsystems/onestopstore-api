    ...
    */
    ...
    public function model(array $row)
    {
        ...
        try {
            ...
            // Handle Product Thumbnail - Store URL directly in attachments
            if (isset($row['product_thumbnail_url']) && !is_null($row['product_thumbnail_url'])) {
                try {
                    $thumbnailUrl = trim($row['product_thumbnail_url']);

                    if (!empty($thumbnailUrl)) {
                        // Create attachment record with URL (no download) - using raw insert like JSON import
                        $attachmentId = \DB::table('attachments')->insertGetId([
                            'file_name' => $thumbnailUrl,
                            'image_url' => $thumbnailUrl,
                            'disk' => 'public',
                            'created_by_id' => \App\Helpers\Helpers::getCurrentUserId() ?? 1,
                            'created_at' => now(),
                            'updated_at' => now(),
                        ]);
                    ...
                    \Log::error("Failed to store product thumbnail URL", [
                        'product_name' => $product->name,
                        'url' => $row['product_thumbnail_url'],
                        'error' => $e->getMessage(),
            ...
            // Handle Product Meta Image - Store URL directly
            if (isset($row['product_meta_image_url']) && !is_null($row['product_meta_image_url'])) {
                try {
                    $metaImageUrl = trim($row['product_meta_image_url']);
                    if (!empty($metaImageUrl)) {
                        $attachmentId = \DB::table('attachments')->insertGetId([
                            'file_name' => $metaImageUrl,
                        ...
                        $product->product_meta_image_id = $attachmentId;

                    }
                } catch (\Exception $e) {
                    \Log::error("Failed to store product meta image URL", [
                        'product_name' => $product->name,
                        'url' => $row['product_meta_image_url'],
                        'error' => $e->getMessage()
                    ]);
                }
            }

            // Handle Size Chart Image - Store URL directly
            if (isset($row['size_chart_image_url']) && !is_null($row['size_chart_image_url'])) {
                try {
                    $sizeChartUrl = trim($row['size_chart_image_url']);
                    if (!empty($sizeChartUrl)) {
                        $attachmentId = \DB::table('attachments')->insertGetId([
                            'file_name' => $sizeChartUrl,
                        ...
                        $product->size_chart_image_id = $attachmentId;

                    }
                } catch (\Exception $e) {
                    \Log::error("Failed to store size chart image URL", [
                        'product_name' => $product->name,
                        'url' => $row['size_chart_image_url'],
                        'error' => $e->getMessage()
                    ]);
                }
            }

            // Save product after setting all attachment IDs (thumbnail, meta, size chart)
            $product->save();

            // Handle Product Galleries - Store URLs directly (comma-separated URLs)
            if (isset($row['product_galleries']) && !is_null($row['product_galleries'])) {
                $product_galleries_urls = array_map('trim', explode(',', $row['product_galleries']));
                $product_galleries_ids = [];


                foreach ($product_galleries_urls as $index => $product_galleries_url) {
                    if (empty($product_galleries_url)) {
                    ...
                    try {
                        // Create attachment record with URL (no download) - using raw insert like JSON import
                        $attachmentId = \DB::table('attachments')->insertGetId([
                            'file_name' => $product_galleries_url,
                            'image_url' => $product_galleries_url,
                            'disk' => 'public',
                            'created_by_id' => \App\Helpers\Helpers::getCurrentUserId() ?? 1,
                            'created_at' => now(),
                            'updated_at' => now(),
                        ...
                        $product_galleries_ids[] = $attachmentId;

                    } catch (\Exception $e) {
                        \Log::error("Failed to store gallery image #{$index} URL", [
                            'product_name' => $product->name,
                            'url' => $product_galleries_url,
                            'error' => $e->getMessage()
                        ]);
                    }
                }

                if (!empty($product_galleries_ids)) {
            ...
            if (isset($row['categories']) && !is_null($row['categories'])) {
                $categoryInput = explode(',', $row['categories']);
                $categoryIds = [];
                ...
                foreach ($categoryInput as $cat) {
                    ...
                    if (is_numeric($cat)) {
                        ...
                        $category = \App\Models\Category::where('name', $cat)
                            ->orWhere('slug', $cat)
                            ->first();
                        ...
                        if ($category) {
                            ...
                            \Log::warning("Category not found during import: {$cat} for product: {$row['name']}");
                        }
                    }
                }
                ...
                if (!empty($categoryIds)) {
                    ...
                    \Log::warning("No valid categories found for product: {$row['name']}");
                }
            }
            ...
            // Update search_keywords after all relationships are attached
            // This ensures categories and tags are included in search
            $product->refresh(); // Reload to get attached relationships
