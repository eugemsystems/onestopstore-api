...
class PayPal {
        ...
        "purchase_units" => [
          [
            ...
            "amount" => [
              ...
              "value"   => Helpers::roundNumber($order->total + ($order->delivery_price ?? 0)),
            ],
  ...
  public static function webhookHandler($request)
  {
    try {
      ...
      // Check if this is a layby payment before processing as regular order
      if ($order && $order->note && preg_match('/TEMP_LAYBY_PAYMENT:(\d+)/', $order->note, $laybyMatches)) {
          $laybyPaymentId = $laybyMatches[1];
          $paymentStatus = match($request->event_type) {
              PaypalEvent::COMPLETED => 'completed',
              PaypalEvent::PENDING => 'pending',
              PaypalEvent::DECLINED, PaypalEvent::CANCELLED => 'failed',
              default => 'failed',
          };
