...
class SignUpBonusPointsNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toWhatsApp(object $notifiable)
    {
        ...
        return [
            'template_id' => config('services.msg91.templates.account_notification'),
            'variables' => [
                $this->user->name,   // {{1}}
                $mainMessage,        // {{2}}
                $additionalInfo      // {{3}}
    ...
    public function tags(): array {
        return ['notify:sign-up-bonus-points', 'user:'.$this->user->id];
    }
}
