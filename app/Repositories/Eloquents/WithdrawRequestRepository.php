...
class WithdrawRequestRepository extends BaseRepository
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            // ✅ FIX: Check for pending withdrawals and subtract from available balance
            // This prevents vendors from requesting more than they have after pending requests
            $pendingWithdrawals = $this->model
                ->where('vendor_id', $vendor_id)
                ->where('status', RequestEnum::PENDING)
                ->sum('amount');

            $availableBalance = $vendorBalance - $pendingWithdrawals;
            ...
            if ($availableBalance < $request->amount) {
                if ($pendingWithdrawals > 0) {
                    throw new Exception("Insufficient available balance. You have $$pendingWithdrawals in pending withdrawal requests. Available balance: $$availableBalance", 400);
                }
            ...
            // ✅ FIX: Do NOT debit wallet on request creation
            // Wallet will be debited when admin APPROVES the request
            // This keeps the balance visible to the vendor until approval
    ...
    public function update($request, $id)
    {
        ...
        try {
            ...
            if (!$withdrawRequest->is_used) {
                // ✅ FIX: Debit wallet when APPROVED (not on request creation)
                if ($withdrawRequest->status == RequestEnum::APPROVED) {
                    $this->debitVendorWallet($withdrawRequest->vendor_id, $withdrawRequest->amount, WalletPointsDetail::WITHDRAW);
                }

                // ❌ OLD LOGIC REMOVED: We don't credit back on rejection anymore
                // because we never debited on request creation
