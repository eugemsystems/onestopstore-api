...
class AdminBackOrderController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            ...
            $permissionMap = [
                'index' => 'back-order.index',
                'upload' => 'back-order.upload',
                'listFiles' => 'back-order.index',
                'process' => 'back-order.process',
                'status' => 'back-order.index',
                'deleteFiles' => 'back-order.delete',
                'clearAll' => 'back-order.delete',
                'history' => 'back-order.index',
            ];

            if (isset($permissionMap[$action])) {
                if (!auth()->user()->can($permissionMap[$action])) {
    ...
    /**
     * Get list of uploaded files (alias for route)
     */
    ...
    /**
     * Process selected files (alias for route)
     */
    ...
    public function processFiles(Request $request)
    {
        ...
        try {
            ...
            Cache::put("backorder_status_{$batchId}", [
                'processed' => 0,
                'updated' => 0,
                'failed' => 0,
                'total' => 0,
            ...
            // Collect all SKUs first to get total count
            $allSkus = [];
            foreach ($selectedFiles as $filePath) {
                if (Storage::disk('local')->exists($filePath)) {
                    $fullPath = Storage::disk('local')->path($filePath);
                    $fileSkus = $this->extractSkusFromFile($fullPath);
                    $allSkus = array_merge($allSkus, $fileSkus);
                }
            }

            $totalSkus = count($allSkus);

            // Update initial status with total
            Cache::put("backorder_status_{$batchId}", [
                'processed' => 0,
                'updated' => 0,
                'failed' => 0,
            ...
            // Process in chunks asynchronously but with larger chunks for better performance
            $chunks = array_chunk($allSkus, 500); // Optimized: 500 SKUs per chunk with raw SQL = very fast

            foreach ($chunks as $chunkIndex => $skuChunk) {
                ProcessBackOrderFile::dispatch($skuChunk, $batchId, 'combined', $chunkIndex);
            }

            return response()->json([
                'success' => true,
                'message' => "Processing started for " . count($selectedFiles) . " file(s) with {$totalSkus} SKUs",
                'batch_id' => $batchId,
    ...
    /**
     * Get processing status (alias for route)
     */
